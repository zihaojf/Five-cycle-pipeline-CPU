  ...
  while (1) {
    ...
    switch (keyid) {
      ...
      case _KEY_BACKSPACE:
        ...
        cur_moveleft();
        for (int32_t pos = pos_cur; pos < pos_end; pos++) {
          vga_putch((uint16_t)(uint8_t)readbuf[pos]);
        }

        pos_end--;
        for (int32_t pos = pos_end + 1; pos > pos_cur; pos--) {
          cur_moveleft();
        }
        break;
      ...
      default: {
        ...
        char ascii = shift_down ? key_id2ascii_shift[keyid] : key_id2ascii[keyid];

        if (capslocked) {
          if (ascii >= 'a' && ascii <= 'z') ascii += (char)('A' - 'a');
          else if (ascii >= 'A' && ascii <= 'Z') ascii += (char)('a' - 'A');
        }

        if (!ascii) break;
        if (pos_end >= READBUF_LEN - 1) break;
