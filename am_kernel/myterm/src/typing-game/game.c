...
// Software frame pacing for FPGA build. This keeps gameplay speed usable
// even when MMIO timer scaling is inaccurate on board.
#define FPGA_FRAME_DELAY_LOOPS 300000u
...
static inline void fpga_frame_wait(void)
{
#ifdef __ARCH_FPGA
  volatile uint32_t n = FPGA_FRAME_DELAY_LOOPS;
  while (n--) {
